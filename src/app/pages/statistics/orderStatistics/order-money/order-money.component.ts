...
@Component({
  selector: 'app-order-money',
  templateUrl: './order-money.component.html',
  styleUrls: ['../../public.scss', './order-money.component.scss'],
  providers: [TabelChartService,JwtService]
})
export class OrderMoneyComponent extends AuthorityComponent implements OnInit {
  linemoneyData: any = [];
  backmoneyinfo: any = [];
  title = "7天订单金额走势";
  toggle = true;
  ...
  constructor(
    private tabelChartService: TabelChartService,
 ...
 if(this.selectedAccounts.length>0) {
   ...
   for (let i = 0; i < this.selectedAccounts.length; i++) {
    ...
    console.log(param);
    this.tabelChartService.QueryForOrder(param).subscribe(data => {
      ...
      this.numtitle = dateRange + "天订单数量走势";
      this.moneytitle = dateRange + "天订单金额走势";
  ...
  initNumChart(list: any, title: any) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      numdate.push(list[i].currentTimeDesc);
    }
  ...
  initMoneyChart(list: any, title: any) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      moneydate.push(list[i].currentTimeDesc);
    }
