...
@Component({
  selector: 'ali-purchase',
  templateUrl: './ali-purchase.component.html',
  styleUrls: ['../../public.scss', './ali-purchase.component.scss'],
  providers: [PurchaseOrder1688Service]
})

export class AliPurchaseComponent extends AuthorityComponent implements OnInit {
  ...
  noLogisticsTime: Array<any> = [{ id: -1, text: '全部' }, { id: 1, text: '1天' }, { id: 2, text: '2天' }, {
    id: 3,
    text: '3天'
  }, { id: 4, text: '3天以上' }];
  paySearch: Array<any> = [{ id: -1, text: '全部' }, { id: 1, text: '未议价' }, { id: 2, text: '议价完成' }, {
    id: 3,
    text: '标记付款'
  }, { id: 4, text: '已付款' }, { id: 5, text: '已拒绝' }];
  ...
  searchKind: Array<any> = [{ id: 1, text: '1688订单号' }, { id: 2, text: '卖家' }, { id: 3, text: '卖家公司名' }, {
    id: 4,
    text: '下单号'
  ...
  searchParam: PlatformOrderModel;
  searchOrderParam: PlatformSearchOrderModel;
  info: Array<any> = [];
  ...
  selectnologitics(val: any) {
    this.selectednologistics = val;
    this.OnQuery();
  }
  ...
  loadAccount() {
    this.purchaseOrderService.GetAliAccounts().subscribe(data => {
      let account = [];
  ...
  OnQuery(pageNumber: number = 1) {
    if (this.clicktop == 0) {
      this.searchParam = new PlatformOrderModel();
      this.searchParam.Account1688Id = this.selected1688;
      ...
      this.searchParam.SendNoLogistics = this.selectednologistics;
      ...
      this.purchaseOrderService.PlatformOrderQuery(this.searchParam).subscribe(data => {
      ...
      this.searchOrderParam = new PlatformSearchOrderModel();
      ...
      this.searchOrderParam.Account1688Id = this.selected1688;
      ...
      this.purchaseOrderService.PlatformOrderSearchQuery(this.searchOrderParam).subscribe(data => {
        this.noPayMoney = data.content.noPayMoney;
        this.payMoney = data.content.payMoney;
        this.totalMoney = data.content.totalMoney;
        this.info = data.content.pageLists.items;
  ...
  openAddRemarksModal(id: any, target: any) {
    ...
    activeModal.result.then(result => {
      if (result != undefined) {
        this.purchaseOrderService.RefuseOrder(id, result).subscribe(data => {
  ...
  openDetailsModal(id: any, purchaseNumber: any, unPackSkuStatusShow) {
    console.log(id);
    ...
    activeModal.componentInstance.unPackSkuStatusShow = unPackSkuStatusShow;
    activeModal.result.then(result => {
    }, reject => {
    });
  }
  ...
  ConfirmPayForOrder(id: any, currentPage) {
    this.confirm("是否确定付款?", () => {
      this.purchaseOrderService.ConfirmPayForOrder(id).subscribe(data => {
        this.reload(currentPage);
      }, this.handleError);
    });
  }
  ...
  batchSyncOrders() {
    ...
    this.confirm("确定批量同步订单?", () => {
      ...
      this.purchaseOrderService.batchSync1688Order(this.selectPackageIds).subscribe(data => {
        ...
        this.reload(this.PageInfo.pageIndex);
      }, e => {
        this.handleError(e);
      });
    })
